...
#################################################################################
@util.flow_logger
def generate_data(train_file, test_given_file, train_start_date, train_end_date, test_start_date, test_end_date):
    ...
    return X, y, test_X, test_y, test_info_map
    ...
    def add_history_travel_info(df):
        ...
        for cur_i, time in enumerate(df['from']):
            ...
            for ind in obj_groups2["{}-{}-{}-{}".format( \
                ...
                if df['from'][ind] < df['from'][cur_i]:
                    his_list.append(df['avg_travel_time'][ind])
            if not his_list == []:
