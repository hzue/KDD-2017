...
class ml:
  ...
  class rf(supervised_learning_interface):
    ...
    def fit(self, X, y):
      self._gen_file(X, "{0}/{1}".format(ml.prefix, self.train_file), y=y)
      feature_num = len(check_output('head -n 1 {0}/{1}'.format( \
                      ml.prefix, self.train_file \
      ...
      check_output('Rscript sbin/rf-train.r regression 400 {0} {1}/{2} {1}/{4} > {1}/{3}'.format( \
                  mtry, ml.prefix, self.train_file, self.score_file, self.model_file
                ), shell=True)

    def predict(self, X):
      self._gen_file(X, "{0}/{1}".format(ml.prefix, self.test_file))
      check_output('Rscript sbin/rf-predict.r regression {0}/{1} {0}/{2} > {0}/{3}'.format( \
                  ml.prefix, self.model_file, self.test_file, self.result_file \
                ), shell=True)
      return self.read_result()

    def read_result(self):
      result_file = open("{0}/{1}".format(ml.prefix, self.result_file), 'r')
  ...
  class svr(supervised_learning_interface):
    ...
    def fit(self, X, y):
      self._gen_file(X, "{}/{}.feature".format(ml.prefix, self.train_file), y=y)
      check_output("svm-train -s 3 {0}/{1}.feature {0}/{2}".format( \
                    ml.prefix, self.train_file, self.model_file \
                ), shell=True)
    ...
    def predict(self, X):
      ...
      check_output("svm-predict {0}/{1}.feature {0}/{2} {0}/{3}".format( \
                  ml.prefix, self.test_file, self.model_file, self.result_file \
                ), shell=True)
      return self.read_result()
